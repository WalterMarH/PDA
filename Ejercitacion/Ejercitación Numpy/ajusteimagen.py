...
def getHistograma(image):
    ...
    historojo=np.histogram2d(image.image[0],bins=lista)
    histoverde=np.histogram(image.image[1],bins=lista)
    histoazul=np.histogram(image.image[2],bins=lista)


    listarojos=historojo[0]
    listaverdes=histoverde[0]
    listaazules=histoazul[0]
    print(listarojos)
